...
We finish with a replication package for a project about public procurement across various European countries. 
...
```{r, child=c(here::here('05-analysis.md'))} 
```

```{r, child=c(here::here('06-robust-code.md'))} 
```

```{r, child=c(here::here('07-mainfile.md'))} 
```
```{r, child=c(here::here('08-secrets-in-code.md'))} 
```

```{r, child=c(here::here('09-versioning.md'))} 
```
